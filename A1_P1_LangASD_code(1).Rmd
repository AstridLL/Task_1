...
data_2a = select(data_2, SUBJ, VISIT, MOT_MLU, MOT_LUstd)
...
```{r}
# removing NA's by repeating values from 1st visit throughout the rest of the visits
# first I will select the rows corresponding to visit 1 in a subset


#data_subset = select(all_data, SUBJ, VISIT, ADOS)
data_ADOS = select(all_data, SUBJ, VISIT == 1, ADOS)

# subset with all ADOS values from first visit 
data_ADOS = subset(all_data, VISIT == 1, ADOS)
data_ADOS = select()
# rename ADOS
data_ADOS = plyr::rename(data_ADOS, c("ADOS"="ADOS_1"))

# merge new ADOS variable with all_data dataset
all_data = merge(all_data,data_ADOS,by="VISIT")

#data_ADOS = rename(data_ADOS, c("ADOS"="ADOS_1"))

#all_data = merge(all_data,data_ADOS,by="VISIT")

#select(all_data$VISIT, contains("1"))
#try=all_data[all_data$VISIT == '1',all_data$ADOS]
#try = all_data[all_data$VISIT==1,all_data$ADOS]
#temporary_data = ifelse(all_data$VISIT == "1", select, NA))

#NO_NA_ADOS = dataa=Data[Data$Pick_danger == 'White shark', -1]

# no_NA_ADOS = all_data$ADOS[all_data$ADOS == 1]

#data_no_NA = select(all_data, SUBJ, ADOS, Verbal_IO, Non_verbal_IQ$)
